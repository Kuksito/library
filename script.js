...
class Book{
    constructor(title, author, published, pages,bookStatus){
        this.title = title;
        this.author = author;
    ...
    *getValues(){
        yield this.title;
        yield this.author;
        yield this.published;
        yield this.pages;
        yield this.bookStatus;
    }
...
function displayBook(){
    ...
    for(let i = 0; i < myLibrary.length; i++){
        newBook.displayBookCard(myLibrary[i])
    }
}
...
function addBookToLibrary(){
    formContainer.addEventListener('submit', (e) => {
        e.preventDefault();  
        newBook = new Book(title.value, author.value, published.value, pages.value, document.querySelector('input[name="book_status"]:checked').value);
